<tr>
    ...
    @if (isset($gallery->image))
        <td><img src="{{ asset(str_replace('\\', '/', $gallery->image)) }}" class="img-circle width-1" alt="images" width="70" height="70"></td>
    @else
...
{{--    <td class="text-center">{{ Carbon\Carbon::parse($gallery->date)->format('Y-m-d') }}</td>--}}
    ...
    <td>
        @if($gallery->is_featured =='1')
            <span class="badge rounded-pill text-bg-info">{{$gallery->is_featured ? 'Yes' : 'No'}}</span>
        @elseif($gallery->is_featured =='0')
            <span class="badge rounded-pill text-bg-danger">{{$gallery->is_featured ? 'Yes' : 'No'}}</span>
        @endif
    </td>
    <td class="text-right">
