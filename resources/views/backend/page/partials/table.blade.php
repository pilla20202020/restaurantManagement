<tr>
    ...
    @if (isset($page->image))
        <td><img src="{{ asset(str_replace('\\', '/', $page->image)) }}" class="img-circle width-1" alt="{{$page->title}}" width="70" height="70"></td>
    @else
    ...
    <td>
        @if($page->is_published =='1')
            <span class="badge rounded-pill text-bg-info">{{$page->is_published ? 'Yes' : 'No'}}</span>
        @elseif($page->is_published =='0')
            <span class="badge rounded-pill text-bg-danger">{{$page->is_published ? 'Yes' : 'No'}}</span>
        @endif
    </td>
    <td class="text-right">
