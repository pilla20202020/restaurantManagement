<tr>
    ...
    @if (isset($volunteer->image))
        <td><img src="{{ asset(str_replace('\\', '/', $volunteer->image)) }}" class="img-circle width-1" alt="{{$volunteer->title}}" width="70" height="70"></td>
    @else
    ...
    <td>
        @if($volunteer->is_featured =='1')
            <span class="badge rounded-pill text-bg-info">{{$volunteer->is_featured ? 'Yes' : 'No'}}</span>
        @elseif($volunteer->is_featured =='0')
            <span class="badge rounded-pill text-bg-danger">{{$volunteer->is_featured ? 'Yes' : 'No'}}</span>
        @endif
    </td>
    <td class="text-right">
